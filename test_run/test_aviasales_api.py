...
import allure
import pytest
from pages.API.data import error_text
from configuration.ConfigProvaider import ConfigProvaider
from testdata.DataProvaider import DataProvaider


key = DataProvaider().get("key")


@pytest.fixture(scope="session")
def browser():
    url = ConfigProvaider().get("api", "base_url")
    return Api(url)
...
def test_cheap_tickets_for_a_certain_date(browser: Api):
    params = {
        'origin': 'MOW',
        'destination': 'YOW',
    ...
    result = browser.cheap_tickets_for_a_certain_date(params)
...
@pytest.mark.api_positive
@pytest.mark.positive
def test_search_for_air_tickets_by_price(browser: Api):
    params = {
        'origin': 'MOW',
        'destination': 'LED',
        'value_min': '1000',
    ...
    result = browser.search_for_air_tickets_by_price(params)
...
@pytest.mark.api_positive
@pytest.mark.positive
def test_air_ticket_prices_for_the_period(browser: Api):
    params = {
        'currency': 'rub',
        'origin': 'MOW',
        'destination': 'YOW',
    ...
    result = browser.air_ticket_prices_for_the_period(params)
...
def test_departure_time_later_than_arrival_time(browser: Api):
    params = {
        'origin': 'MOW',
        'destination': 'NQZ',
...
@pytest.mark.api_negative
@pytest.mark.negative
def test_departure_and_arrival_cities_are_not_specified(browser: Api):
    params = {
